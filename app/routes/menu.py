...
class MenuController:
    ...
    def get_menu_or_404(self, restaurant_id: int, day: str, current_user: dict):
    ...
    def get_menu(self, restaurant_id: int, day: str, current_user: dict = Depends(get_current_restaurant)):
        return self.get_menu_or_404(restaurant_id, day, current_user)

    def update_menu(self, restaurant_id: int, day: str, menu: MenuUpdate, current_user: dict = Depends(get_current_restaurant)):
        self.get_menu_or_404(restaurant_id, day, current_user)
        return self.menu_service.update_menu(restaurant_id, day, menu)

    def delete_menu(self, restaurant_id: int, day: str, current_user: dict = Depends(get_current_restaurant)):
        self.get_menu_or_404(restaurant_id, day, current_user)
...
def get_menu(restaurant_id: int, day: str, controller: MenuController = Depends(get_controller)):
    return controller.get_menu(restaurant_id, day)

@router.put("/{restaurant_id}/{day}", response_model=MenuCreate)
def update_menu(restaurant_id: int, day: str, menu: MenuUpdate, controller: MenuController = Depends(get_controller)):
    return controller.update_menu(restaurant_id, day, menu)

@router.delete("/{restaurant_id}/{day}")
def delete_menu(restaurant_id: int, day: str, controller: MenuController = Depends(get_controller)):
